...
- **GitHub Repository 동기화**: 백업된 포스트를 GitHub private repo에 자동 커밋
- **이메일 알림**: 백업 완료/실패 시 이메일로 알림 수신 (GitHub repo, Velog, 대시보드 링크 포함)
...
4. Repository는 **private**으로 자동 생성되어 백업 내용이 비공개로 보호됩니다

### 6. 이메일 알림 (선택)
- 설정 페이지에서 "백업 알림" 토글 활성화
- 백업 완료/실패 시 등록된 이메일로 알림 수신
- 이메일에 관련 서비스 링크 포함:
  - **GitHub Repository**: 동기화된 repo 바로가기 (동기화 활성화 시)
  - **Velog Blog**: 본인 블로그 바로가기
  - **Dashboard**: 대시보드 바로가기
...
A: 로그인 시 부여된 GitHub 권한으로 지정한 repo에 포스트를 자동 커밋합니다. repo가 없으면 **private**으로 자동 생성됩니다.
